...
public class DownloadItemHolder extends LocalItemHolder {
    ...
    public DownloadItemHolder(LocalItemBuilder infoItemBuilder, ViewGroup parent) {
    ...
    private String getStreamInfoDetailLine(final DownloadEntry entry,
                                           final DateFormat dateFormat) {
        final String uploadDate = dateFormat.format(entry.downloadDate);
        final String serviceName = NewPipe.getNameOfService(entry.serviceId);
        return Localization.concatenateStrings(uploadDate, serviceName);
    }

    @Override
    public void updateFromItem(final LocalItem localItem, final DateFormat dateFormat) {
        if (!(localItem instanceof DownloadEntry)) return;
        final DownloadEntry item = (DownloadEntry) localItem;
